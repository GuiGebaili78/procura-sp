...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const actions = {
    ...
    setEndereco: (endereco: ViaCepResponse | null) => setState(prev => ({ ...prev, endereco })),
    setCoordenadas: (coordenadas: { lat: number; lng: number } | null) => setState(prev => ({ ...prev, coordenadas })),
    setSelectedService: (selectedService: string) => setState(prev => ({ ...prev, selectedService })),
    setSearchResults: (searchResults: CataBagulhoResult[]) => setState(prev => ({ ...prev, searchResults })),
    setIsLoading: (isLoading: boolean) => setState(prev => ({ ...prev, isLoading })),
    setError: (error: string | null) => setState(prev => ({ ...prev, error })),
    setSearchCompleted: (searchCompleted: boolean) => setState(prev => ({ ...prev, searchCompleted })),
    resetSearch: () => setState(prev => ({
      ...prev,
      searchResults: [],
      coordenadas: null,
      error: null,
