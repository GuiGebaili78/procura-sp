          ...
          <div className="flex-1">
            ...
            <div className="text-sm text-gray-600 space-y-1">
              {service.startStretch && (
                <p><span className="font-medium">Início:</span> {service.startStretch}</p>
              )}
              {service.endStretch && (
                <p><span className="font-medium">Fim:</span> {service.endStretch}</p>
              )}
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
          {service.frequency && (
            <div className="bg-gray-50 rounded-lg p-3">
              <span className="text-xs font-medium text-gray-500 block mb-1">Frequência</span>
              <span className="text-sm text-dark-primary">{service.frequency}</span>
            </div>
          )}
          
          {service.shift && (
            <div className="bg-gray-50 rounded-lg p-3">
              <span className="text-xs font-medium text-gray-500 block mb-1">Turno</span>
          ...
          {service.schedule && (
            <div className="bg-gray-50 rounded-lg p-3 sm:col-span-2">
              <span className="text-xs font-medium text-gray-500 block mb-1">Horário</span>
              <span className="text-sm text-dark-primary">{service.schedule}</span>
            </div>
          )}
        </div>
