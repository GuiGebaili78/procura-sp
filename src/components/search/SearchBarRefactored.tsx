...
export function SearchBarRefactored({ 
  ...
  const enderecoCompleto = useMemo(() => {
    ...
    console.log("📍 [SearchBar] Endereço completo construído:", enderecoCompleto);
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      console.log("📍 [SearchBar] Tentando geocoding real do endereço...");
      console.log("📍 [SearchBar] Endereço completo:", enderecoCompleto);
      console.log("📍 [SearchBar] CEP:", cep, "Número:", numero);
      
      let coordinates: { lat: number; lng: number } | null = null;
      
      try {
        console.log("🔍 [SearchBar] Tentando geocoding para:", enderecoCompleto);
        coordinates = await geocodeAddress(enderecoCompleto, cep, numero);
        console.log("🔍 [SearchBar] Resultado do geocoding:", coordinates);
        
        if (!coordinates) {
          console.log("⚠️ [SearchBar] Geocoding retornou resultado vazio, usando coordenadas aproximadas por CEP");
          coordinates = obterCoordenadasAproximadasPorCEP(cep) || null;
        }
      } catch (error) {
        console.error("❌ [SearchBar] Erro no geocoding:", error);
        console.log("⚠️ [SearchBar] Geocoding falhou, usando coordenadas aproximadas por CEP");
        coordinates = obterCoordenadasAproximadasPorCEP(cep) || null;
        console.log("📍 [SearchBar] Coordenadas aproximadas obtidas:", coordinates);
      }
      
      // Verificar se as coordenadas são válidas
      if (coordinates && (coordinates.lat === -23.5505 && coordinates.lng === -46.6333)) {
        console.warn("⚠️ [SearchBar] Coordenadas são da Praça da Sé - possível fallback indevido");
      }
      
      if (!coordinates) {
        console.error("❌ [SearchBar] Nenhuma coordenada obtida, nem por geocoding nem por fallback");
        throw new Error(
          "Não foi possível encontrar as coordenadas do endereço. Verifique se o CEP e número estão corretos.",
        );
      }
      
      console.log("✅ [SearchBar] Coordenadas finais obtidas:", coordinates);
      
      // Verificar se as coordenadas finais são válidas
      if (coordinates && (coordinates.lat === -23.5505 && coordinates.lng === -46.6333)) {
        console.warn("⚠️ [SearchBar] ATENÇÃO: Coordenadas finais são da Praça da Sé - possível fallback indevido");
        console.warn("⚠️ [SearchBar] CEP:", cep, "Número:", numero);
        console.warn("⚠️ [SearchBar] Endereço completo:", enderecoCompleto);
      }
      ...
      console.log("🚀 [SearchBar] - Resultados:", results);
      console.log("🚀 [SearchBar] - Coordenadas:", coordinates);
      console.log("🚀 [SearchBar] - Tipo de serviço:", selectedService);
      console.log("🚀 [SearchBar] - Endereço:", enderecoCompleto);
      
      // Verificar se as coordenadas finais são válidas antes de enviar
      if (coordinates && (coordinates.lat === -23.5505 && coordinates.lng === -46.6333)) {
        console.warn("⚠️ [SearchBar] ATENÇÃO: Enviando coordenadas da Praça da Sé para o componente pai!");
        console.warn("⚠️ [SearchBar] Isso pode causar o problema do marcador no lugar errado!");
        console.warn("⚠️ [SearchBar] CEP:", cep, "Número:", numero);
        console.warn("⚠️ [SearchBar] Endereço completo:", enderecoCompleto);
      }
